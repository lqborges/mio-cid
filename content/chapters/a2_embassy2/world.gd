extends Node3D
## a2_embassy2: Medinaceli road escort and second gift. Keep Avengalvón.

const BEAT_ID := &"a2_embassy2"
const DEST := &"a2_jeronimo"
const HUB_LOCK := &"hub_lock_cardena"
const HORSE_FLAG := "horse_companion"
const DONE_FLAG := "embassy2_done"
const RECRUIT_FLAG := "avengalvon_recruited"
const ESCORT_FLAG := "medinaceli_escort_done"
const FAMILY_FLAG := "family_in_valencia"
const AVENGALVON_ID := &"avengalvon"
const GIFT_PATH := "res://data/gifts/embassy_2.json"
const DIALOGUE_PATH := "res://content/chapters/a2_embassy2/embassy2.dialogue"
const BALLOON_PATH := "res://game/ui/talk_balloon.tscn"
const PROMPT_KEY := "a2_embassy2.escort_prompt"
const RECRUIT_KEY := "a2_embassy2.recruit"
const NEED_KEY := "a2_embassy2.need_avengalvon"
const LEAVE_KEY := "a2_embassy2.alvar_leave"
const RETURN_KEY := "a2_embassy2.alvar_return"
const FAMILY_KEY := "a2_embassy2.family_arrives"
const BLOCKED_KEY := "ui.embassy_ledger.blocked"
const PLACE_KEY := "a2_embassy2.place_name"
...
var _recruited: bool = false
var _escorted: bool = false
...
func start_recruit(_cue: String = "recruit") -> void:
	if _recruited or _talking:
		return
	_talking = true
	...
	if resource == null:
		...
		recruit_avengalvon()
		return
	if _try_balloon(resource, "recruit"):
		return
	_walk_recruit(resource)


func recruit_avengalvon() -> void:
	if _recruited:
		return
	_recruited = true
	var roster: Variant = _roster()
	if roster and roster.has_method("add_member"):
		if not (roster.has_method("member") and roster.member(AVENGALVON_ID) != null):
			var member := MesnadaMember.from_id(AVENGALVON_ID)
			if member:
				roster.add_member(member)
	_set_flag(RECRUIT_FLAG)
	_whisper(RECRUIT_KEY)
	_whisper("a2_embassy2.not_creed")


func complete_escort() -> bool:
	if _escorted:
		return true
	if not _recruited:
		_whisper(NEED_KEY)
		return false
	_escorted = true
	_set_flag(ESCORT_FLAG)
	_whisper("a2_embassy2.jimena_road")
	return true
...
func complete_return() -> void:
	if _returned:
		return
	if not _gifted or not _recruited:
		return
	_alvar_return()
	_set_return_flags()
	_leave_for_hub()
...
func avengalvon_kept() -> bool:
	if not _recruited:
		return false
	var body: Node = get_node_or_null("Avengalvon")
	...
	if member == null or not bool(member.alive):
		return false
	return bool(member.essential) and String(member.must_survive_until) == "a3_despedida"
...
func _set_return_flags() -> void:
	...
	ChapterRunner.add_flag(FAMILY_FLAG)
	if _recruited:
		ChapterRunner.add_flag(RECRUIT_FLAG)
	if _escorted:
		ChapterRunner.add_flag(ESCORT_FLAG)


func _leave_for_hub() -> void:
	var travelled := false
...
func _connect_zones() -> void:
	_connect_zone("RecruitZone", _on_recruit_entered)
	_connect_zone("EscortEnd", _on_escort_entered)
...
func _on_recruit_entered(body: Node) -> void:
	if _is_player_or_horse(body):
		start_recruit()


func _on_escort_entered(body: Node) -> void:
	if _is_player_or_horse(body):
		complete_escort()
...
func _walk_recruit(resource: Resource) -> void:
	await _walk_lines(resource, "recruit")
	_talking = false
	recruit_avengalvon()
