extends Control
## Mesura hold vs rage dump. Spanish labels; English loc keys.
...
func _apply_copy() -> void:
	...
	var rage: Button = get_node_or_null("Panel/Rage") as Button
	if prompt:
		prompt.text = _loc("a3_leon.choice_prompt")
	if mesura:
		mesura.text = _loc("a3_leon.choice_mesura")
	if rage:
		rage.text = _loc("a3_leon.choice_rage")
