...
class TestPlayerGuide(unittest.TestCase):
    def test_autoload_and_pause_pages(self) -> None:
        ...
        self.assertIn("func help_lines(", guide)
        self.assertIn("WASD — ", guide)
        self.assertIn("hud.wasd", guide)
        self.assertIn("hud.move_click", guide)
        self.assertIn("func _overlay_busy(", guide)
        self.assertIn("PRESET_CENTER_TOP", guide)
        ...
        self.assertIn("_set_talk_panels_visible", pause)
        self.assertIn("_set_interact_prompt_visible", pause)
        self.assertIn('begins_with("WASD")', pause)
        self.assertIn("hud.wasd", pause)
        self.assertIn('_make_button("Settings"', pause)
        self.assertIn("grab_focus", pause)
        self.assertIn("_clear_action_queues", pause)
        self.assertIn("release_pointer", pause)
        self.assertIn("settings", _read("game/ui/pause_menu.gd"))
        self.assertIn("note_moved", guide)
        self.assertIn('_complete_tip_if("moved")', guide)
        ...
        self.assertIn("func release_pointer(", _read("game/ui/touch_hud.gd"))
        hud_src = _read("game/ui/touch_hud.gd")
        pause_hook = hud_src.split("func _on_pause_pressed", 1)[1].split("func _wire_buttons", 1)[0]
        self.assertLess(pause_hook.find("release_pointer"), pause_hook.find('call("open")'))
        ...
        self.assertIn("ui.menu.overwrite", locales)
        self.assertIn("ui.menu.cancel", locales)
        ...
        self.assertIn("hud.onores.gloss", locales)
        self.assertIn("hud.honor.gloss,rey,king", locales)
        self.assertIn("hud.honra.gloss,nombre,name", locales)
        self.assertIn("hud.wasd,WASD — Andar,WASD — Walk", locales)
    ...
    def test_controller_uses_glyphs_and_target(self) -> None:
        ...
        self.assertIn("_place_prompt_over", source)
        self.assertIn("unproject_position", source)
        self.assertIn("InteractChip", source)
        self.assertIn("Label3D", source)
        self.assertIn("SIZE_SHRINK_CENTER", source)
        self.assertIn('name = "InteractPrompt"', source)
        self.assertIn("get_tree().root", source)
