...
func _check_scene_loads() -> PackedStringArray:
	...
	if ResourceLoader.exists(WAIT):
		failures.append("a3_valencia_wait must not ship")
	if ResourceLoader.exists(CARRION):
		failures.append("a3_carrion must not ship")
	return failures
...
func _check_dest_missing_no_scene_change() -> PackedStringArray:
	...
	if ResourceLoader.exists(WAIT):
		failures.append("a3_valencia_wait must not ship")
	if bool(world.get("_left")):
		failures.append("must not _left into missing a3_valencia_wait")
	if _runner and String(_runner.current_id) == "a3_valencia_wait":
		failures.append("must not travel into missing a3_valencia_wait")
	if current_scene != scene_before:
		failures.append("win must not change_scene to wait")
	if bool(world.call("try_travel_wait")):
		failures.append("try_travel_wait must no-op when dest is missing")
	if current_scene != scene_before:
	...
	if _completed.count("a3_toledo") < 1:
		failures.append("missing dest must still complete a3_toledo once")
	world.free()
	return failures
