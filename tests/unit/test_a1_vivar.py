...
class TestA1VivarPrologue(unittest.TestCase):
    ...
    def test_hud_loc_and_pause_copy(self) -> None:
        ...
        self.assertEqual(rows["hud.onores.gloss"]["es"], "feudos")
        self.assertEqual(rows["hud.honor.gloss"]["es"], "rey")
        self.assertEqual(rows["hud.honra.gloss"]["es"], "nombre")
        self.assertEqual(rows["hud.wasd"]["es"], "WASD — Andar")
        self.assertIn("WASD", rows["hud.controls_hint"]["es"])
        meters_scene = _read("game/ui/honor_meters.tscn")
        self.assertIn("GlossOnores", meters_scene)
        self.assertIn("feudos", meters_scene)
        self.assertIn("nombre", meters_scene)
        self.assertIn("Caption", _read("game/ui/plazo_bar.tscn"))
